...
const popular = ["Smoked sausages", "Beef hot dogs", "Minced beef"];

/** Bone sheet that drops from the top. Filters products by name on the client. */
export function SearchSheet() {
  ...
  useEffect(() => {
    if (!isOpen) return;
    const t = setTimeout(() => input.current?.focus(), 240);
    return () => clearTimeout(t);
  }, [isOpen]);

  const term = q.trim().toLowerCase();
  const hits = term ? products.filter((p) => `${p.name} ${p.kind}`.toLowerCase().includes(term)) : [];
    ...
    <div className={`fixed inset-0 z-70 ${isOpen ? "visible opacity-100" : "pointer-events-none invisible opacity-0"}`} aria-hidden={!isOpen}>
      ...
      <div className={`absolute top-0 right-0 left-0 bg-bone px-[clamp(18px,4vw,46px)] py-[clamp(34px,6vh,64px)] text-ink transition-transform duration-600 ease-lm ${isOpen ? "translate-y-0" : "-translate-y-full"}`}>
        <div className="mx-auto flex max-w-[1180px] items-center gap-5 border-b-[1.5px] border-ink">
          ...
          />
          <button onClick={close} aria-label="Close search" className="flex size-[52px] shrink-0 items-center justify-center text-ink">
            <CloseIcon />
          </button>
        </div>
        ...
        {term && (
          <ul className="mx-auto mt-4 max-w-[1180px] divide-y divide-ink/15">
            {hits.length === 0 && <li className="py-3 text-bone-muted">No match. Try “sausages” or “mince”.</li>}
            {hits.map((p) => (
              <li key={p.id}>
                <Link href={`/range#${p.id}`} onClick={close} className="flex items-baseline justify-between gap-4 py-3 hover:text-brass-deep-2">
                  ...
                  <span className="font-mono text-xs text-bone-muted">{ugx(p.p500)} / 500g</span>
                </Link>
              </li>
            ))}
          </ul>
        ...
        <div className="mx-auto mt-5 flex max-w-[1180px] flex-wrap items-center gap-2.5">
          <span className="mr-1.5 font-mono text-[11px] tracking-[.14em] text-bone-muted uppercase">Popular</span>
          {popular.map((label) => (
