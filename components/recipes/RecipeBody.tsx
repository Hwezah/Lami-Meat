...
export function RecipeBody({ r, tone, labels }: { r: Recipe; tone: "dark" | "bone"; labels: { youNeed: string; method: string } }) {
  const dark = tone === "dark";
  const label = `mb-2.5 font-mono text-[11px] uppercase tracking-[.14em] ${dark ? "text-dark-muted" : "text-bone-muted"}`;
  return (
    <div className={`grid items-start gap-x-[clamp(28px,4vw,64px)] gap-y-7 text-start max-sm:grid-cols-1 ${dark ? "grid-cols-[1fr_1.2fr]" : "grid-cols-[minmax(0,1fr)_minmax(0,1.3fr)]"}`}>
      <div className={dark ? "" : "bg-bone-2 p-[clamp(18px,2vw,26px)]"}>
        <div className={label}>{dark ? labels.youNeed : `${labels.youNeed} · ${r.serves}`}</div>
      ...
      <div>
        <div className={label}>{labels.method}</div>
