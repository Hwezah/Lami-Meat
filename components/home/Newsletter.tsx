...
export function Newsletter() {
  ...
  if (done) return <div className="border-b border-brass py-3.5 text-[15.5px] text-bone">You&apos;re on the list — thank you.</div>;
  ...
  return (
    <form onSubmit={subscribe} noValidate>
      <div className="flex border-b border-brass mp:flex-col mp:items-stretch mp:gap-2.5">
        ...
        />
        <button type="submit" className="inline-flex items-center gap-2 py-3.5 pr-1 pl-4 font-display text-[13.5px] font-extrabold tracking-[.1em] text-brass uppercase mp:mx-auto mp:w-[80vw] mp:max-w-full mp:justify-center">
          Subscribe →
        </button>
      </div>
      {bad && <p className="mt-2.5 text-[13px] text-error-dark mp:text-center">Please enter a valid email address.</p>}
    </form>
  );
}
