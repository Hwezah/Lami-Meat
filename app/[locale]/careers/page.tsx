...
import { roles as enRoles } from "@/data/roles";
import { getPageDict, pageMetadata, type LocaleParams } from "@/lib/i18n/server";
import { site } from "@/lib/config";
import { wa } from "@/lib/whatsapp";

export const generateMetadata = (props: LocaleParams) => pageMetadata(props, "careers", "/careers");
...
export default async function CareersPage(props: LocaleParams) {
  ...
  const C = t.careers;
  /** Role titles in emails and WhatsApp stay in English for the team. */
  const enTitle = (id: string) => enRoles.find((r) => r.id === id)?.title ?? id;
  return (
    <>
      <PageHeader eyebrow={C.eyebrow} line1={C.title1} line2={C.title2}>
        <Ticker lines={t.tickers.careers} />
      </PageHeader>
        ...
        <div className="lm-wrap grid grid-cols-3 gap-[clamp(10px,1.4vw,18px)] max-md:grid-cols-1">
          {photos.map((src, i) => (
            <div key={src} data-reveal data-reveal-delay={i * 90} className="relative aspect-[4/5]">
              <Photo src={src} alt={t.common.teamAlt} sizes="(max-width:760px) 100vw, 33vw" />
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="lm-section mp:text-center">
        <div className="lm-wrap">
          <div className="mb-[clamp(36px,5vw,60px)] max-w-[720px]">
            <Eyebrow className="mb-4">{C.whyEyebrow}</Eyebrow>
              ...
              items={C.roles.map((r) => ({
                ...
                body: (
                  <>
                    ...
                    <div className="mt-[22px] mb-1.5 font-mono text-[11px] tracking-[.14em] text-bone-muted uppercase">{C.lookFor}</div>
                    ...
                    <div data-cta-actions className="mt-6 flex flex-wrap gap-2.5">
                      <a href={`mailto:${site.email}?subject=${encodeURIComponent(C.subject(enTitle(r.id)))}`} className={`${cta.ink} ${ctaSm}`}>
                        <span data-full>{C.apply}</span>
                        <span data-short>{C.applyShort}</span>
                      </a>
                      <a href={wa(t.wa.role(enTitle(r.id)))} target="_blank" rel="noopener" className={`${cta.outlineInk} ${ctaSm}`}>
                        <span data-full>{C.ask}</span>
                        <span data-short>{C.askShort}</span>
                      </a>
                    </div>
                  </>
                ),
